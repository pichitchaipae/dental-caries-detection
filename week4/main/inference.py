"""
Two-Stage Dental Caries Detection System with Instance Segmentation (v3.1 - High Accuracy)
================================================================================
...
Date: 2025-01-28
Version: 3.1 (High Accuracy Configuration)
"""
...
# =============================================================================
# CONFIGURATION - HIGH ACCURACY PARAMETERS
# =============================================================================
...
HIGH_ACCURACY_CONFIG = {
    # Caries detection - lower threshold to catch more lesions
    'caries_confidence': 0.01,      # Very low to maximize recall
    'caries_iou_threshold': 0.3,    # Lower IoU for NMS to keep more detections
    'caries_max_det': 100,          # Maximum detections per image
    
    # Tooth detection - balanced for accuracy
    'tooth_confidence': 0.35,       # Lower threshold for better tooth coverage
    'tooth_iou_threshold': 0.4,     # Standard IoU threshold
    
    # Image processing
    'image_size': 1280,             # Larger image size for better accuracy
    'augment': True,                # Test-time augmentation
    
    # Matching parameters
    'min_iou_for_match': 0.05,      # Minimum IoU to consider a match
    'max_distance_for_match': 0.20, # Maximum normalized distance for fallback
}
...
# Default paths - Fixed to week4 folder structure
WEEK4_DIR = Path(__file__).parent.parent  # week4 folder
DEFAULT_TOOTH_MAP_PATH = WEEK4_DIR / 'tooth_map_config.json'
DEFAULT_TOOTH_MODEL_PATH = WEEK4_DIR.parent / 'material' / 'Tooth Segmentation + Recognition model' / 'weights' / 'Tooth_seg_pano_20250319.pt'
# Detectron2 crop segmentation model path
DEFAULT_CROP_MODEL_PATH = WEEK4_DIR.parent / 'material' / 'Tooth Segmentation + Recognition model' / 'weights' / 'Tooth_seg_crop_20250424.pth'

# Default output directory - Fixed to week4
DEFAULT_OUTPUT_DIR = WEEK4_DIR / 'inference_output'

# Crop padding for fine segmentation
CROP_PAD = 25  # Increased padding for better context
...
def load_detectron2_predictor(model_path: str, score_thresh: float = 0.3) -> Optional[Any]:
    """
    Load Detectron2 predictor for fine-grained tooth segmentation.
    
    Args:
        model_path: Path to the crop segmentation model weights (.pth)
        score_thresh: Confidence threshold for predictions (lowered for high accuracy)
...
def detect_teeth(
    ...
    Args:
        ...
        confidence_threshold: Minimum confidence for detections (None = use HIGH_ACCURACY_CONFIG)
        detectron_predictor: Optional Detectron2 predictor for fine segmentation
        use_high_accuracy: Whether to use high accuracy settings
    ...
    # Use high accuracy config if enabled
    if confidence_threshold is None:
        confidence_threshold = HIGH_ACCURACY_CONFIG['tooth_confidence'] if use_high_accuracy else 0.5
    
    # Run inference with high accuracy settings
    predict_kwargs = {
        'source': image,
        'conf': confidence_threshold,
        'verbose': False,
        'iou': HIGH_ACCURACY_CONFIG['tooth_iou_threshold'] if use_high_accuracy else 0.5,
    }
    
    if use_high_accuracy:
        predict_kwargs['imgsz'] = HIGH_ACCURACY_CONFIG['image_size']
        predict_kwargs['augment'] = HIGH_ACCURACY_CONFIG['augment']
...
def find_tooth_for_caries_dynamic(
    ...
    Args:
        ...
        min_iou_threshold: Minimum IoU for valid match (None = use HIGH_ACCURACY_CONFIG)
    ...
    if min_iou_threshold is None:
        min_iou_threshold = HIGH_ACCURACY_CONFIG['min_iou_for_match']
    ...
    if best_tooth and best_iou > min_iou_threshold:
...
def find_tooth_for_caries_static(
    ...
    Args:
        ...
        max_distance: Maximum allowed distance for valid match (None = use HIGH_ACCURACY_CONFIG)
    ...
    if max_distance is None:
        max_distance = HIGH_ACCURACY_CONFIG['max_distance_for_match']
...
def detect_caries(
    ...
    # Use high accuracy config if enabled
    if confidence_threshold is None:
        confidence_threshold = HIGH_ACCURACY_CONFIG['caries_confidence'] if use_high_accuracy else 0.25
    
    # Build prediction kwargs with high accuracy settings
    predict_kwargs = {
        'source': image,
        'conf': confidence_threshold,
        'verbose': False,
        'iou': HIGH_ACCURACY_CONFIG['caries_iou_threshold'] if use_high_accuracy else 0.5,
        'max_det': HIGH_ACCURACY_CONFIG['caries_max_det'] if use_high_accuracy else 300,
    }
...
def run_inference(
    ...
    detectron_predictor: Optional[Any] = None,
    caries_confidence: float = None,
    tooth_confidence: float = None,
    visualize: bool = True,
    use_high_accuracy: bool = True
) -> InferenceResult:
    """
    ...
    Args:
        ...
        detectron_predictor: Optional Detectron2 predictor for fine segmentation
        caries_confidence: Confidence threshold for caries detection (None = use HIGH_ACCURACY_CONFIG)
        tooth_confidence: Confidence threshold for tooth detection (None = use HIGH_ACCURACY_CONFIG)
    ...
    print(f"Mode: {'HIGH ACCURACY' if use_high_accuracy else 'STANDARD'}")
    print(f"{'='*70}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Dental Caries Detection System (v3.1 - High Accuracy)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Basic inference with high accuracy (default)
  ...
  python inference.py -i ./test.jpg -m ./caries_model.pt --standard

Pipeline (v3.1 High Accuracy):
  Stage 1:   YOLO Caries Detection (3-class) - conf=0.01, augment=True
  Stage 2a:  YOLO Panoramic Tooth Detection - conf=0.35
  Stage 2b:  Crop tooth regions with padding (25px)
...
Caries Classes:
  ...
  2: Lingual   - Caries on tongue-facing surface
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help=f'Confidence threshold for caries detection (default: {HIGH_ACCURACY_CONFIG["caries_confidence"]} for high accuracy)'
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help=f'Confidence threshold for tooth detection (default: {HIGH_ACCURACY_CONFIG["tooth_confidence"]} for high accuracy)'
    )
    ...
    parser.add_argument(
        '--standard',
        action='store_true',
        help='Use standard accuracy mode (faster but less sensitive)'
    )
    
    args = parser.parse_args()
    
    # Determine accuracy mode
    use_high_accuracy = not args.standard
    ...
    # =========================================================================
    print("\n" + "="*70)
    print("DENTAL CARIES DETECTION SYSTEM v3.1 (High Accuracy)")
    print("="*70)
    print(f"Mode: {'HIGH ACCURACY' if use_high_accuracy else 'STANDARD'}")
    if use_high_accuracy:
        print(f"  → Caries confidence: {HIGH_ACCURACY_CONFIG['caries_confidence']}")
        print(f"  → Tooth confidence:  {HIGH_ACCURACY_CONFIG['tooth_confidence']}")
        print(f"  → Image size:        {HIGH_ACCURACY_CONFIG['image_size']}")
        print(f"  → Test-time augment: {HIGH_ACCURACY_CONFIG['augment']}")
