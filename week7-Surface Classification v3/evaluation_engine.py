...
# Change this to your base directory containing the "material", "week2", "week3", "week5", and "week7" folders.
BASE_DIR = Path(r"C:\Users\jaopi\Desktop\SP")

# Inputs
MATERIAL_DIR   = BASE_DIR / "raw_data" / "500 cases with annotation(xml)" # XML ground truth
WEEK2_DIR      = BASE_DIR / "week2-Tooth Detection & Segmentation"  / "500-segmentation+recognition" # tooth polygons for MZ reclassification
WEEK5_DIR      = BASE_DIR / "week5-Surface Classification v1"  / "surface_classification_output" # original predictions (before week7 fixes)
...
# Output
WEEK7_DIR      = BASE_DIR / "week7-Surface Classification v3"
...
def normalize_surface(raw: str) -> str:
    ...
    CRITICAL RULE: Only three surface classes are permitted.
    Any unapproved surface (Buccal, Lingual, Palatal, etc.) is
    mapped to 'Unclassified'.
    """
    r = raw.strip().lower()
    if "occlusal" in r:
        return "Occlusal"
    if "mesial" in r:          # catches "mesial", "proximal-mesial"
        return "Mesial"
    if "distal" in r:          # catches "distal", "proximal-distal"
        return "Distal"
    # Generic "proximal" without M/D qualifier → Unclassified (ambiguous)
    # Buccal, Lingual, Palatal, Cervical, Other → Unclassified
    return "Unclassified"
...
# NOTE: Only {Mesial, Distal, Occlusal} are permitted surface classes.
_ADJACENT_SURFACES = {
