...
MIN_CLUSTER_SIZE        = 15
DEBUG_DIR               = Path(r"C:\Users\jaopi\Desktop\SP\week7-Surface Classification v3\evaluation_output")
...
def classify_multi_zone(
    ...
    **week7 change:** Uses ``_pca_rotation_angle_fixed()`` which
    ...
    # Surface Incorrect -> (Distal, Mesial, Occlusal) only, no other.
    total_occ  = zone_counts["M"] + zone_counts["C"] + zone_counts["D"]
    total_prox = zone_counts["Proximal_L"] + zone_counts["Proximal_R"]

    # NOTE: primary_surface is kept internally for zone_label logic,
    # but final output will only ever be Distal / Mesial / Occlusal.
    ...
    # RULE: Only Distal / Mesial / Occlusal are permitted.
    #       Proximal → remap to Mesial or Distal; Other → dominant zone fallback.
    if _internal_surface == "Occlusal":
        ...
        # Proximal surfaces are remapped to Mesial or Distal.
        ...
        # "Other" fallback: use all-points X-distribution to pick
        # the dominant zone among M / C / D (computed below).
    ...
    if zone_label == "_DEFERRED":
        ...
        zone_label = _fallback  # "M", "C", or "D"

    # ── Remap primary_surface to allowed classes ─────────────────────
    # Allowed: Distal, Mesial, Occlusal only.
    if _internal_surface == "Proximal":
        ...
        primary_surface = {"M": "Mesial", "C": "Occlusal", "D": "Distal"}[zone_label] if zone_label in ("M", "C", "D") else "Occlusal"
    else:
