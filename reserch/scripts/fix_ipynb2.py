...
for cell in data["cells"]:
    if "source" in cell and any("def match_case" in line for line in cell["source"]):
        ...
        new_func = """def match_case(gt, pred):
    ...
    return y_true, y_pred"""
        ...
        # Replace just the match_case part
        import re
        src = re.sub(r"def match_case\(gt, pred\):.*?(?=\n\n\n# =========================================================\n# MAIN LOOP)", new_func, src, flags=re.DOTALL)
        
        # Split back to lines
        lines = [line + "\n" for line in src.split("\n")]
        # remove the extra newline added to the very last line
        if lines:
            lines[-1] = lines[-1][:-1]
